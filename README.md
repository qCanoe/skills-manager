...
[概览](#概览) • [功能特性](#功能特性) • [快速开始](#快速开始) • [使用方式](#使用方式) • [文件夹与浏览模式](#文件夹与浏览模式) • [Skill 目录结构](#skill-目录结构) • [开发说明](#开发说明) • [Roadmap](#roadmap)
...
- **文件夹（本机 Collections）**：在来源区切换「来源 / 文件夹」浏览模式；可新建、重命名、删除文件夹，并用自定义文件夹选择器切换当前文件夹；列表与预览按当前文件夹筛选；在 skill 预览工具栏的「文件夹」菜单中勾选加入/移出各文件夹，**无文件夹时仍可打开该菜单**，并通过 **新建** 创建文件夹（可将当前 skill 一并加入）；数据仅存 `localStorage`（以 `sourceId` + 相对路径引用 skill，**不复制磁盘文件**）；复制目标与合并路径展示会隐藏当前索引为 0 个 skill 的来源，与列表筛选一致
...
在来源区域顶部使用分段控件在 **来源** 与 **文件夹** 之间切换（对应实现中的 *collections*，界面文案为「文件夹」）。

### 行为概要

| 模式 | 列表内容 |
| --- | --- |
| **来源** | 按已启用来源扫描得到的全部 skill（可再配合搜索、仅可写等筛选） |
| **文件夹** | 仅显示**当前选中文件夹**内引用的 skill；未选文件夹时会显示空状态提示 |

### 管理文件夹

- **新建 / 重命名 / 删除**：在「文件夹」浏览模式下通过工具栏与对话框完成；删除文件夹会移除其中的引用，不会删除磁盘上的 `SKILL.md`。
- **当前文件夹**：使用自定义 **文件夹选择** 控件（下拉面板）切换；列表与预览状态随当前文件夹变化。

### 将 skill 加入或移出文件夹

- 在 **skill 详情预览** 中打开 **文件夹** 按钮下的菜单：对每个文件夹使用复选框 **加入 / 移出**。
- 菜单顶部提供 **新建**：打开应用内命名对话框创建文件夹；若当前有选中的 skill，创建后会**自动将该 skill 加入新文件夹**。
- 在「文件夹」模式下，若当前 skill 属于正在查看的文件夹，可使用 **从当前文件夹移除**（仅移除引用）。

### 数据与限制

- 文件夹与成员关系保存在浏览器 **`localStorage`**（本机、按配置域隔离），**不会**把文件复制到新目录；与真实 `SKILL.md` 仍是引用关系。
- 若某来源当前索引为 0 个 skill，在复制目标列表、来源芯片等位置会隐藏该来源，避免空来源干扰操作。
...
- [x] **本机文件夹（Collections）**：浏览模式、文件夹 CRUD、预览侧加入/移出与新建文件夹、空状态与复制目标过滤
